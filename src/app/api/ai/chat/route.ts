...
export async function POST(request: NextRequest) {
  try {
    ...
    const systemPrompt = getChatSystemPrompt({
      ...
      hasReee: clientInfo?.has_reee ?? false,
      hasCeliapp: clientInfo?.has_celiapp ?? false,
      celiappBalance: clientInfo?.celiapp_balance ?? null,
      hasCri: clientInfo?.has_cri ?? false,
      criBalance: clientInfo?.cri_balance ?? null,
      hasFrv: clientInfo?.has_frv ?? false,
      frvBalance: clientInfo?.frv_balance ?? null,
      riskScore: riskAssessment?.risk_score ?? 5,
      riskProfile: riskAssessment?.risk_profile ?? "modéré",
